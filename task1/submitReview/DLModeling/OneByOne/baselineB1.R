...
finalData   <- finalData[intersectionTollgate=='B1',]
...
##################################################################
search_criteria = list(strategy = "RandomDiscrete", max_models = 30, max_runtime_secs = 1500, seed=123456)
...
modelResult 	<- head(DLGridResult,10);
#modelResult     <- modelResult[-c(3,4,7,8),]
...
write.table(ATTNNSubmit,file=paste(Sys.Date(), "OBOB1.csv", sep=""),  row.names=F,col.names=F, quote=F, sep=",")

save(ATTNNSubmit,DLGridResult,modelResult,modelSummary,file=paste(Sys.Date(), "OBOB1.Rdata", sep=""))
