...
def main():
    ...
    data = feature_transform_gbrt(key=1, data=data)

    y = data.ix[:len_train - 1]["volume"]
    x = data.ix[:len_train - 1, 8:]
    x1 = data.ix[len_train:, 8:]

    regressor_cubic = GradientBoostingRegressor(n_estimators=250, learning_rate=0.05, min_samples_split=15,
                                                min_samples_leaf=2, max_depth=5, max_features='sqrt', subsample=0.8,
    ...
    df_test.to_csv("result/result_gbrt_"+str(np.mean(yhat))+".csv", index=False)
